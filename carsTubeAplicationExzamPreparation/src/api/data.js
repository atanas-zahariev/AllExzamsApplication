...
const endpoints = {
    ...
    logout: '/users/logout',
    getAllDataInSistem: '/data/cars?sortBy=_createdOn%20desc',
    getSpecificDataWithId:'/data/cars/',
    addInSysten:'/data/cars'
}
...
export async function getUserList(userId){
    const result = await get(`/data/cars?where=_ownerId%3D%22${userId}%22&sortBy=_createdOn%20desc`)
    return result;
}

export async function searchByQuery(query){
    const result = await get(`/data/cars?where=year%3D${query}`);
    return result;
}